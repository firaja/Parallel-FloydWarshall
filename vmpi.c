// mpicc -g -Wall vmpi.c -o mpi.out -O3 && mpirun -np 5 vmpi.out 20 100
#include <stdio.h>
#include <stdlib.h>
#include <time.h>
#include <mpi.h>
...
void populateMatrix(int matrix[], int n, int density, int rank, int processes) { 
	...
	int rem = (n*n)%processes;
	int sum = 0;

	int *sendcounts = malloc(sizeof(int)*processes);
    int *displs = malloc(sizeof(int)*processes);

    for (int i = 0; i < processes; i++) 
    {
        sendcounts[i] = (n*n)/processes;
        if (rem > 0) 
        {
            sendcounts[i]++;
            rem--;
	...
	if (rank == ROOT) {
		...
		MPI_Scatterv(temp_mat, sendcounts, displs, MPI_INT, matrix, sendcounts[rank], MPI_INT, 0, MPI_COMM_WORLD);
...
void gatherResult(int matrix[], int n, int rank, int processes) 
{
	...
	if (rank == ROOT) {
		temp_mat = malloc(n * n * sizeof(int));
		int split = n * n / processes;
		MPI_Gatherv(matrix, sendcounts[ROOT], MPI_INT, temp_mat, sendcounts, displs, MPI_INT, 0, MPI_COMM_WORLD);
	...
	{
		int split = n * n / processes;
		MPI_Gatherv(matrix, sendcounts[rank], MPI_INT, temp_mat, sendcounts, displs, MPI_INT, 0, MPI_COMM_WORLD);
	}
}
