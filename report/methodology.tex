...
In recent years, many scientific and numeric GPGPU applications found success due to graphics hardware’s streaming data-parallel organizational model. With the introduction of NVIDIA G80 GPU architecture, the graphics pipeline
now features a single unified set of processors that function as vertex, geometry, and fragment processors. Additionally, the release of the Compute Unified Device Architecture
(CUDA) API [NVI07] on the G80 architecture allows developers to easily develop and manage general purpose scientific and numerical algorithms without formulating solutions
in terms of nontrivial shaders and graphic primitives.


The GPU serves, to an extent, as a coprocessor to the
CPU programmed through the CUDA API. A single program know as a kernel is compiled to operate on the GPU
...
data through a high-speed shared memory region (16 kB in
size) on the G80 architecture accessible to the programmer
...
manage this ultra fast shared memory cache effectively. This
...
so that no data race occurs but each thread instead of covering one or more rows, is assigned to a single cell. Whereas each block of threads covers horizzontally the
entire matrix and vertically a proportional number of rows, depending on the number of threads per block.
...
$i \leftarrow D_{y}(B) \times B_y + T_y$ \\
$j \leftarrow D_{x}(B) \times B_x + T_x$ \\
\If{$x < n \wedge y < n$}{
...
\caption{Kernel of the \emph{CUDA-FW}}\label{alg:cuda}
\end{algorithm}

Because each thread does little computation and access memory, timings are really low if compared
with \emph{serial-FW} and even \emph{OMP} FW with 8 cores. \\
In fact from \textbf{Figure \ref*{fig:cuda-time}} we see a speedup up to $87.8$ times when comparing with \emph{serial} FW
and $12.34$ with \emph{OMP} FW with 8 cores. We also notice that the maximum speedup is reached when the number of threads per block is equal to 128;
